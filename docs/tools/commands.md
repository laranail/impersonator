...
Six Artisan commands. All named `laranail::impersonator.<command>`, per the laranail convention.
...
Checks the master switch, boot health, tamper-evidence configuration, all four tables, the target
allowlist against what actually resolved, every configured guard, the driver and adapter, the
enter-plus-mode permission trap, the gate ability, whether a revocation can genuinely end a session,
the maximum duration, the approval setup, the API's auth stack, and other impersonation packages.
