...
`null` auto-selects: `RbacPolicy` when spatie/laravel-permission is installed, `BasePolicy`
otherwise. Name a class to take over — extend one of them rather than reimplementing, so the
always-on identity rules cannot be lost by accident.
...
when the model exposes them. That is spatie's shape but also several other packages', and it means
no hard dependency and no `class_exists` dance. A model with none of those methods inherits the
base behaviour.
