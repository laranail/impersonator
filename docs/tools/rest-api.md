...
| `POST` | `/impersonations/current/extend` | the mode permission, re-checked |
...
```json
{ "granted": true, "expires_at": "2026-08-12T10:20:00+00:00", "seconds": 600, "extensions": 1 }
```

`seconds` is what was **actually** added, not what was configured — the last extension before the
ceiling is clamped rather than refused, so a client should render this value rather than assume the
window length.

**403** with a `reason` on refusal: `extension_limit`, `extension_ceiling`, `extension_too_early`,
`extension_disabled`, `not_impersonating`, or `session_terminated`. As everywhere else in this API,
branch on `reason` and not on `message`.

No `{audit}` parameter, deliberately: an operator may extend the session they are in and no other.
Applied inside a locked transaction, so a client retrying spends one allowance rather than two, and
permission is re-checked — extending asks for *more* access, unlike leaving. See
...
| 403 | Refused — `reason` carries a stable code (including every extension refusal) |
...
```php
'rate_limiting' => ['api' => ['attempts' => 30, 'decay' => 60]],
```
