...
Every participant is stored as a morph pair rather than a foreign key, because the allowlist is
multi-model — one trail holds impersonations of `User`, `Vendor` and anything else registered.

| Pair | Who |
|---|---|
| `impersonator_type` / `impersonator_id` | The operator |
| `impersonatable_type` / `impersonatable_id` | The impersonated account |
| `revoked_by_type` / `revoked_by_id` | Whoever pulled the kill switch, if anyone |

The impersonated account is named for the ability that makes it eligible (`canBeImpersonated()`)
rather than the role it plays in one request. `target` remains the name in the API, in the Core value
object and on the `?target=` filter — those are about a request, not a column.

The `*_type` half always holds a **morph alias**, never a class name. That is what keeps a crafted
`target_type` from naming an arbitrary class, and it is why a renamed class does not orphan its
history. See [`morphs`](../configuration.md#polymorphic-columns) for the `*_id` column type and for
requiring aliases application-wide.

The `*_id` half is a **string** by default, so one table can hold an int-keyed `User` beside a
UUID-keyed `Vendor`. `Identity` compares ids loosely on their PHP type to match.
