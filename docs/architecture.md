...
`src/Core` depends on `psr/log`, `psr/clock`, `psr/event-dispatcher` and nothing else. It imports
no `Illuminate` code at all, and an architecture test asserts this on every commit:
...
Not portability for its own sake — a Symfony bridge is a nice property, not the point. The
