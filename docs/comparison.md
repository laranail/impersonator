...
> **A note on the sixth column.** It was originally listed as `victoryoalli/laravel-impersonate`, which
> does not exist — the package under that vendor is
> [`victoryoalli/laravel-multitenancy-impersonate`](https://github.com/victoryoalli/laravel-multitenancy-impersonate),
> and its scope is narrower than the name suggested: landlord→tenant handoff, with a hard dependency on
> `spatie/laravel-multitenancy`. Every cell below was read from v2.4.0's source rather than inferred.
>
> Judged on its own terms it does the thing it sets out to do — a single-use UUID token with a TTL, rate
> limiting, and a redirect into the tenant domain. The empty cells are features it does not claim, not
> failures.

| | **laranail** | lab404 | octopyid | stechstudio (Filament) | victoryoalli (multitenancy) | stancl built-in |
|---|---|---|---|---|---|---|
...
| Session-level audit trail | ✓ | — | — | — | partial | — |
...
`src/Core` imports no `Illuminate` code and calls no framework helper, enforced by architecture
tests. The practical payoff is not portability — Core does import `laranail/enumerator` for enum
labels, and that package's own dependencies include Illuminate — it is that the whole authorization
decision is testable without a request, a session or a container, which is the only reason the suite
can carry as many refusal tests as it does. See
[Architecture](architecture.md#the-layers) for what that does and does not guarantee.
