...
Two independent narrowings apply: `ability` names the single capability the token carries, and
`mode` bounds what the impersonated session may do regardless of that ability. Set both — the
ability is what a resource server checks, the mode is what this package enforces.
