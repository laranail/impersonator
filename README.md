...
Requires PHP 8.4.1+ and Laravel 13. Hard dependencies are `illuminate/*`, three PSR interfaces, and
four `laranail/*` packages. Sanctum, Passport, JWT, stancl/tenancy and spatie/laravel-permission are
all optional — install one and its integration activates itself.
...
The `laranail/*` family resolves through git rather than Packagist, so the closure has to be declared
...
The alias is registered by package discovery. If your application disables aliases, import the
facade class itself instead:
...
Thirty pages, in [`docs/`](docs/) — they render on GitHub, so every link below works from here.
The hosted copy at
[opensource.simtabi.com](https://opensource.simtabi.com/documentation/laranail/impersonator/) is
the canonical home once this package is picked up by the docs aggregator.
...
- [Events](docs/tools/events.md) — fifteen, including the refusals
...
- [Extend a running impersonation](docs/recipes/extend-a-running-impersonation.md)
- [Scope rows correctly under PostgreSQL RLS](docs/recipes/scope-rows-with-postgres-rls.md)
...
vendor/bin/pest                          # add IMPERSONATOR_TEST_DB=pgsql|mysql for a real driver
