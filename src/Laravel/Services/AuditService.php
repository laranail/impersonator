...
final readonly class AuditService
{
    ...
    public function query(array $filters = []): Builder
    {
        ...
        // The filter name and the column prefix are deliberately separate arguments. `target` is
        // the public API field and stays that way; the column it reads is `impersonatable_*`.
        // Deriving one from the other is what broke this filter when the columns were renamed.
        $this->applyIdentity($query, 'impersonator', $filters['impersonator'] ?? null);
        $this->applyIdentity($query, 'impersonatable', $filters['target'] ?? null);
