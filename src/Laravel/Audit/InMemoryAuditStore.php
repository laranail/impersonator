...
final class InMemoryAuditStore implements AuditStore
{
    ...
    public function extend(string $auditId, ExtensionPolicy $policy, DateTimeImmutable $now): ExtensionOutcome
    {
        ...
        // The revocation flag lives beside the rows here rather than on them, so it has to
        // be folded in before the policy is asked — otherwise a revoked impersonation looks
        // extendable and this fake disagrees with the real store on the one case that matters.
        $session = isset($this->revocations[$auditId]) ? $row->revoked($now) : $row;
        $grant = $policy->evaluate($session, $now);

        if ($grant->denied() || $grant->expiresAt === null) {
            return new ExtensionOutcome($session, $grant);
        }

        $this->rows[$auditId] = $row->extended($grant->expiresAt);

        return new ExtensionOutcome($this->rows[$auditId], $grant);
    }
