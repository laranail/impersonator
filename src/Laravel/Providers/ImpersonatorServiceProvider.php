...
class ImpersonatorServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        $failures->critical('impersonator.boot.persistence_guard', $this->registerPersistenceGuard(...));
        $failures->critical('impersonator.boot.morph_map', $this->registerMorphMap(...));

        $failures->degradable('impersonator.boot.routes', $this->registerRoutes(...));
        $failures->degradable('impersonator.boot.translations', $this->registerTranslations(...));
        ...
        if ($this->app->runningInConsole()) {
            ...
            $failures->degradable('impersonator.boot.doctor_checks', $this->registerDoctorChecks(...));
        }
    }
    ...
    protected function registerSupport(): void
    {
        ...
        // One armed-state object per application, consulted by the single
        // `DB::beforeExecuting` listener registered in boot(). See PersistenceGuard for why
        // the listener cannot be per-request.
        $this->app->singleton(PersistenceGuard::class, static fn (): PersistenceGuard => new PersistenceGuard);
        ...
        // Singleton: it holds the runtime eligibility closure, and a fresh instance per resolution
        // would quietly drop a rule an application registered in a provider's boot.
        $this->app->singleton(
            ReviewerDirectory::class,
            static fn (Application $app): ReviewerDirectory => new ReviewerDirectory($app->make(Settings::class)),
        );

        $this->app->singleton(
            MessageCatalog::class,
            static fn (Application $app): MessageCatalog => new MessageCatalog($app->make(Translator::class)),
        );
    ...
    protected function registerContracts(): void
    {
        ...
        $this->app->singleton(
            ...
            static fn (Application $app): ApprovalStore => new EloquentApprovalStore(
                $app->make(ClockInterface::class),
                $app->make(Settings::class),
                // The audit connection, so a chain lives beside the trail it belongs to.
                $app->make(ConnectionResolverInterface::class)->connection(
                    $app->make(Settings::class)->nullableString('audit.connection'),
                ),
            ),
        );
     ...
     *
     * How "installed" is decided is the application's to change: a class list in
     * `authorization.rbac.detect`, or `Impersonator::detectRbacUsing()` for a rule config cannot
     * express. See {@see ImpersonationManager::detectsRbac()}.
     */
     ...
     * English literal built in Core, so an application adding a locale can translate as much or as
     * little as it likes and the untranslated remainder stays readable.
     */
    ...
    protected function registerEventListeners(): void
    {
        ...
        // Every event, not a chosen few. A dispatched event written nowhere is the gap that made
        // `ModeViolationBlocked` — a session probing its own boundary — invisible outside the request
        // that produced it.
        /*
         * Between-request resets, for Octane and any other long-running process.
         *
         * Listened by event **name** rather than by class: these strings resolve only when Octane is
         * installed and dispatching them, so there is no `class_exists` probe and no dependency. Both
         * boundaries are covered because a request that dies hard never reaches `RequestTerminated`.
         */
        foreach ([
            ...
            $events->listen($octaneEvent, [ResetImpersonatorState::class, 'handle']);
        }
    ...
    protected function registerRateLimiters(): void
    {
        ...
        // Keyed on the operator, not `$request->user()`. While impersonating those are different
        // people, and billing an operator's attempts to the customer's bucket lets one operator
        // rate-limit a chosen customer out of their own application.
        $caller = static function (Request $request) use ($app): string {
            $operator = $app->make(ImpersonationManager::class)->rateLimitKey($request);
     ...
     */
    /**
     * One `DB::beforeExecuting` listener for the whole application.
     *
     * Registered once at boot rather than per request, because `beforeExecuting` has no removal
     * counterpart — a per-request listener would outlive its request, deny the audit UPDATE that
     * closing an impersonation performs, and trap the operator inside the account. It does nothing
     * at all until the mode middleware arms {@see PersistenceGuard}, so applications that never use
     * a persistence-guarding mode pay one closure call per write and no more.
     *
     * Critical rather than degradable: this listener *is* the read-only guarantee. A read-only mode
     * whose guard silently failed to register would report a restriction it is not applying, which
     * is worse than not offering the mode.
     */
    protected function registerPersistenceGuard(): void
    {
        $guard = $this->app->make(PersistenceGuard::class);
        $events = $this->app->make(Dispatcher::class);
        $catalog = $this->app->make(MessageCatalog::class);

        DB::beforeExecuting(static function (string $query, array $bindings, $connection) use ($guard, $events, $catalog): void {
            $decision = $guard->inspect($query);
            ...
            if ($session !== null) {
                $events->dispatch(new ModeViolationBlocked(
                    $session,
                    AttemptedAction::write('unknown', 'write'),
                    $decision,
                ));
            }

            throw new HttpException(403, $catalog->forDecision($decision));
        });
    }
    ...
    protected function registerExceptionRendering(): void
    {
        ...
        // Every closure below renders the *translated* message and the *untranslated* code. That
        // split is the published contract: `reason` is stable and branchable, `message` is display
        // text that moves with the locale.
        $handler->renderable(static function (ImpersonationDenied $e, Request $request) use ($catalog) {
            $message = $catalog->forDecision($e->decision);

            return $request->expectsJson()
                ? response()->json(['message' => $message, 'reason' => $e->code()], 403)
                : response($message, 403);
        });

        $handler->renderable(static function (ConcurrencyLimitReached $e, Request $request) use ($catalog) {
            $message = $catalog->forDecision($e->decision);

            return $request->expectsJson()
                ? response()->json(['message' => $message, 'reason' => $e->decision->code], 429)
                : response($message, 429);
        });

        $handler->renderable(static function (TokenRejected $e, Request $request) use ($catalog) {
            // Deliberately uniform, and translated through ONE key. `reason()` — unknown, expired,
            // already_used, revoked — reaches the audit log and never the response: telling a caller
            // that a token merely expired tells them the token was real.
            $message = $catalog->forException('token_rejected', $e->getMessage());
        ...
        $handler->renderable(static function (ApprovalRequired $e, Request $request) use ($catalog) {
            $message = $catalog->forException($e->code(), $e->getMessage());
        ...
        $handler->renderable(static function (ApprovalNotDecidable $e, Request $request) use ($catalog) {
            ...
            return $request->expectsJson()
                ? response()->json(['message' => $message, 'reason' => $e->reason()], 409)
    ...
    protected function registerAboutCommand(): void
    {
        ...
        AboutCommand::add('Impersonator', fn (): array => [
            ...
            'Extension' => ! $settings->bool('limits.extension.enabled', true)
                ...
                : sprintf(
                    '+%d min, %s, ceiling %s',
                    $settings->int('limits.extension.minutes', 10),
                    ($count = $settings->positiveIntOrNull('limits.extension.max')) === null
                        ? 'unlimited'
                        : sprintf('max %d', $count),
                    ($total = $settings->positiveIntOrNull('limits.extension.max_total_duration')) === null
                        ? 'none'
                        : sprintf('%d min', $total),
                ),
    ...
    /**
     * Opt the application into requiring a morph alias for every polymorphic type.
     *
     * `Relation::requireMorphMap()` makes Eloquent throw rather than fall back to storing a fully
     * qualified class name in a `*_type` column. Two things that buys, and they are worth
     * distinguishing:
     *
     *  - **The column stops leaking the application's internals.** A `*_type` holding
     *    `App\Models\Billing\LegacyCustomer` puts the class map into every audit export.
     *  - **A renamed or moved class stops silently orphaning history.** An alias survives a
     *    namespace change; a class name does not, and the row it broke is an audit record.
     *
     * **Off by default because the call is application-global, not scoped to this package.**
     * Turning it on makes unmapped morphs throw anywhere in the host application, which is a good
     * practice but not one a package installs on somebody's behalf — an upgrade that started
     * throwing on an unrelated `commentable` would be this package's fault and not its business.
     *
     * Critical rather than degradable: a half-applied morph policy is worse than either state.
     * If the application asked for aliases to be mandatory and the call silently failed, class
     * names would be written into the audit trail while the configuration said otherwise — and
     * nothing downstream would notice until an export was read months later.
     *
     * The package's own boundary does not depend on this. `targets.allowlist` is deny-by-default,
     * so an unaliased class cannot be impersonated whatever this is set to; see
     * {@see TargetRegistry}.
     */
    protected function registerMorphMap(): void
    {
        // Only the global flag here. Publishing the allowlist's own aliases belongs to
        // TargetRegistry, which does it as it resolves — reading the registry from boot memoises it
        // against boot-time config, so any later `registerTarget()` or config change was silently
        // discarded for the rest of the request.
        if ($this->app->make(Settings::class)->bool('morphs.require_map', false)) {
            Relation::requireMorphMap();
        }
    }

    /**
     * Publish this package's diagnostics into the family-wide doctor.
     *
     * The same {@see Checks} objects `laranail::impersonator.doctor` runs, so the two reports cannot
     * disagree — and an operator running `laranail::package-tools.doctor` across every installed
     * laranail package sees impersonation's findings alongside the rest, which is the point.
     *
     * Guarded on `bound()` rather than `class_exists()`: package-tools is a hard dependency, so the
     * class is always there, but its provider may not have registered the singleton yet — or at all,
     * in a host that excluded it from discovery. A missing registry means no shared doctor, not a
     * boot failure.
     *
     * **Instances, not class names**, and **console only.** `DoctorService::register()` instantiates a
     * string argument with a bare `new $check` — no container, no arguments — so a check taking
     * `Settings` in its constructor would raise an ArgumentCountError at boot. Resolving them here
     * keeps constructor injection, and confining it to the console means the cost is paid by the one
     * process that can actually run a doctor rather than by every web request.
     *
     * Constructing them is cheap regardless: a {@see Check} constructor only stores two references,
     * and every expensive thing — the container lookups, the schema probe — happens inside `run()`.
     *
     * Degradable: losing the shared doctor loses a diagnostic, not a control. The package's own
     * command still works, and `register()` de-duplicates on (group, name) so a double boot cannot
     * stack the report.
     */
    protected function registerDoctorChecks(): void
    {
        ...
        foreach (Checks::all() as $class) {
            ...
            // Verified rather than trusted: `make()` is `mixed`, and a host application may rebind
            // any of these. A rebinding that no longer satisfies the contract is skipped rather than
            // handed to a registry that would throw for it later, mid-report.
            if ($check instanceof DoctorCheck) {
                $service->register($check, 'laranail/impersonator');
            }
        }
    }
