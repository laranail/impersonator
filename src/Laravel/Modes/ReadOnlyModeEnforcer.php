 ...
 *
 * Method checking cannot see a write reached through a GET route, so
 * `modes.read_only.prevent_writes` adds a persistence-level net. That is the
 * stricter setting and it is off by default; see the middleware for why.
 */
final readonly class ReadOnlyModeEnforcer implements ModeEnforcer
{
    ...
    public function check(AttemptedAction $action, ImpersonationSession $session): Decision
    {
        ...
        if ($action->modelClass !== null && $action->normalizedMethod() !== 'ABILITY') {
            return $this->isAllowedRoute($action)
                ...
                : Decision::deny(
                    ...
                    ['model' => $action->modelClass, 'operation' => $action->normalizedMethod()],
                );
        }
        ...
        return Decision::deny(
            ...
            ['method' => $action->normalizedMethod(), 'route' => $action->routeName, 'path' => $action->path],
        );
    }

    public function guardsPersistence(): bool
    {
        return $this->settings->bool('modes.read_only.prevent_writes', false);
    }
