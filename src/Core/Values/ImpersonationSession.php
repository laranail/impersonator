...
final readonly class ImpersonationSession
{
    ...
    /**
     * The same impersonation, running until later.
     *
     * The only non-terminal transition on this object, and it moves exactly two fields. The
     * expiry is not among the facts the audit hash chain covers (see
     * `EloquentAuditStore::chainFacts()`), so extending leaves tamper verification intact —
     * which is the property that makes an extendable window acceptable in an audited system
     * at all.
     */
    public function extended(DateTimeImmutable $expiresAt): self
    {
        return new self(
            $this->auditId,
            $this->impersonator,
    ...
    /**
     * Seconds left before this expires, or null when it never does.
     *
     * Floored at zero rather than going negative: a countdown is the only consumer, and a
     * negative remainder renders as a growing number counting the wrong way.
     */
