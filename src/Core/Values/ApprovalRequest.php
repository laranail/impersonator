...
final readonly class ApprovalRequest
{
    public function __construct(
        ...
        /**
         * The decision that **closed** the chain — the reviewer who denied it, or the last one to
         * approve. A rollup for display and for the API's existing shape, not the record: the full
         * chain is {@see ApprovalStore::decisions()}.
         *
         * Null on a partially approved request, deliberately. Naming one reviewer there would read as
         * "this was decided by them" when the request is still waiting on somebody else.
         */
    ...
    /** The same request with its closing-decision rollup filled in. @see ApprovalStore::decisions() */
    public function withDecider(Identity $decidedBy, ?string $note): self
    {
        return new self(
            $this->id,
            $this->requester,
