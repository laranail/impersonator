...
beforeEach(function (): void {
    Schema::create('users', function (Blueprint $table): void {
        ...
        // Needed by the remember-token regression test: leaving must not rotate the
        // target's own recaller, which would log the real customer out everywhere.
...
// ── session data must not cross the boundary ────────────────────────────────

it('does not carry the operator session payload into the impersonated session', function (): void {
    // Rotating the session id is not enough: `migrate()` keeps `$attributes`, so without a flush
    // the operator's cart, half-finished form or flashed data travels into the target's session.
    // This is the leak octopyid documents as "the cart bleeds back into the admin's session".
    Auth::guard('web')->setUser($this->admin);
    ...
    expect(session()->has('cart'))->toBeFalse()
        ->and(session()->has('wizard.step'))->toBeFalse()
        ->and(Auth::guard('web')->id())->toBe($this->target->getKey());
});

it('does not carry the target session payload back out on leave', function (): void {
    // The other direction, which matters more: whatever the target accumulated during the
    // support session must not land in the operator's own session afterwards.
    Auth::guard('web')->setUser($this->admin);
    Impersonator::enter($this->target);

    session()->put('cart', ['target-item']);
    session()->put('viewed_invoice', 991);

    Impersonator::leave();

    expect(session()->has('cart'))->toBeFalse()
        ->and(session()->has('viewed_invoice'))->toBeFalse()
...
it('can be told to keep the session payload across a switch', function (): void {
    ...
    // The documented opt-out, and the documented consequence: the target inherits it.
    expect(session()->get('cart'))->toBe(['operator-item']);
});

it('mints a fresh csrf token on every switch', function (): void {
    // A token minted at one privilege level must not still be valid at another.
    Auth::guard('web')->setUser($this->admin);
    $before = session()->token();

    Impersonator::enter($this->target);
    $during = session()->token();

    Impersonator::leave();
    $after = session()->token();

    expect($during)->not->toBe($before)
        ->and($after)->not->toBe($during);
});

it('never rotates the target remember token when leaving', function (): void {
    ...
    expect($this->target->fresh()->remember_token)->toBe('customer-token-set-weeks-ago');
});
